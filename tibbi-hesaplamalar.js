(function () {
  ...
  function adminIcerigiAyarla(admin, durum) {
    const gate = $('#medcalcGate');
    const gateMetin = $('#medcalcGateMetin');
    const giris = $('#medcalcGateGiris');
    document.querySelectorAll('[data-admin-icerik]').forEach((node) => {
      node.hidden = admin !== true;
    });
    if (gate) gate.hidden = admin === true;
    if (gateMetin && admin !== true) {
      gateMetin.textContent = durum === 'cikis'
        ? 'Bu sayfa yalnızca admin hesabı ile görüntülenebilir. Lütfen admin hesabınızla giriş yapın.'
        : durum === 'hazir'
          ? 'Bu sayfayı görüntülemek için admin yetkisi gerekir. Bu hesapta admin yetkisi görünmüyor.'
        : 'Bu sayfa yalnızca admin hesabı ile görüntülenebilir. Oturum ve yetki doğrulanıyor.';
    }
    if (giris) giris.hidden = admin === true;
    if (admin === true && !adminIcerigiAyarla.renderEdildi) {
      adminIcerigiAyarla.renderEdildi = true;
      render();
    }
  }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (ara) {
      ara.addEventListener('input', () => {
        arama = ara.value || '';
        if (window.__AHEK_WEB_ADMIN_MI === true) render();
      });
    }
    if (document.body.classList.contains('medcalc-admin-sayfasi')) {
      adminIcerigiAyarla(window.__AHEK_WEB_ADMIN_MI === true, 'kontrol');
      window.addEventListener('ahek-web-admin-durum', (e) => {
        adminIcerigiAyarla(e.detail?.admin === true, e.detail?.durum || '');
      });
      return;
    }
    render();

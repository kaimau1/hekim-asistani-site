...
function profiliYansit(veri = {}) {
  ...
  if (durumMetin) {
    durumMetin.textContent = dogrulanmis
      ...
      : 'Premium deneme ve eklenti bağlantısı için e-posta adresinizi doğrulayın.';
  }
...
async function epostaDogrulamaGonder() {
  ...
  if (btn) btn.disabled = true;
  profilEpostaDogrulamaDurumYaz('Doğrulama e-postası gönderiliyor...', 'bilgi');
  try {
    await sendEmailVerification(user);
    profilEpostaDogrulamaDurumYaz('Doğrulama bağlantısı gönderildi. Gelen kutunuzu ve spam klasörünü kontrol edin.', 'basari');
  } catch (err) {
...
async function epostaDogrulamaKontrolEt() {
  ...
  if (btn) btn.disabled = true;
  profilEpostaDogrulamaDurumYaz('E-posta doğrulama durumu kontrol ediliyor...', 'bilgi');
  try {
    await reload(user);
    if (auth.currentUser?.emailVerified === true) {
      await auth.currentUser.getIdToken(true).catch(() => null);
      profilEpostaDogrulamaDurumYaz('E-posta doğrulandı. Eklentiye tekrar giriş yapabilirsiniz.', 'basari');
      profilEpostaDogrulamaGorunumunuAyarla();
    ...
    profilEpostaDogrulamaDurumYaz('Henüz doğrulanmamış görünüyor. Maildeki bağlantıya tıkladıktan sonra tekrar deneyin.', 'hata');
  } catch (err) {

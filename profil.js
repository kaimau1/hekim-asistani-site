...
function uyelikAksiyonunuAyarla(tier) {
  ...
  aksiyon.hidden = premiumMi || !_aktifUid;
...
function profilCkysDuzeltmeMailOlustur(ckysKodu) {
  const user = auth.currentUser || null;
  const email = user?.email || _sonProfilVerisi.email || '';
  const uid = user?.uid || _aktifUid || '';
  const eskiCkys = String(_sonProfilVerisi.ckysKodu || '').replace(/\D+/g, '');
  const subject = 'Ahek Plus ÇKYS kodu düzeltme talebi';
  const body = [
    'Merhaba,',
    '',
    'Ahek Plus Premium deneme üyeliğimde ÇKYS kodumu yanlış yazdım. Manuel düzeltme rica ederim.',
    '',
    `Doğru ÇKYS kodu: ${ckysKodu}`,
    `Kayıtlı/eski ÇKYS kodu: ${eskiCkys || 'Bilinmiyor'}`,
...
function profilCkysDuzeltmeGonder(e) {
  e.preventDefault();
  ...
  window.location.href = profilCkysDuzeltmeMailOlustur(ckysKodu);
  profilCkysDuzeltmeDurumYaz('E-posta hazırlandı. Metinde hesap e-postası, kullanıcı id ve doğru ÇKYS kodu var.', 'bilgi');
}

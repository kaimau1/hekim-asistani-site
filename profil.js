...
import {
  getAuth, onAuthStateChanged, signOut, deleteUser, sendEmailVerification, reload
...
function profilOzetCacheUygula(uid = '') {
  ...
  maasModuluLinkleriniAyarla(maasModuluYetkiliMi(cache));
  return true;
}
...
function maasModuluYetkiliMi(veri = {}) {
  return true;
}

function maasModuluLinkleriniAyarla(yetkili = false) {
...
function epostaDogrulanmisMi(user = auth.currentUser) {
  return user?.emailVerified === true;
}

function profilEpostaDogrulamaDurumYaz(metin, tip = '') {
  const alan = el('profilEpostaDogrulamaDurum');
  if (!alan) return;
  alan.textContent = metin || '';
  alan.className = 'form-durum' + (tip ? ' ' + tip : '');
}

function profilEpostaDogrulamaGorunumunuAyarla() {
  const kart = el('profilEpostaDogrulama');
  if (!kart) return;
  const user = auth.currentUser;
  const dogrulamaGerekli = !!user && !epostaDogrulanmisMi(user);
  kart.hidden = !dogrulamaGerekli;
  if (!dogrulamaGerekli) profilEpostaDogrulamaDurumYaz('');
}
...
function profilGorunumuAyarla(girisVarMi, adminYetkili = false) {
  ...
  profilEpostaDogrulamaGorunumunuAyarla();
}
...
function profilKartlariniTemizle() {
  ...
  maasModuluLinkleriniAyarla(false);
...
function profiliYansit(veri = {}) {
  ...
  const durumBaslik = el('profilDurumBaslik');
  const durumMetin = el('profilDurumMetin');
  const dogrulanmis = epostaDogrulanmisMi(user);
  if (durumBaslik) durumBaslik.textContent = dogrulanmis ? 'Giriş yapıldı' : 'E-posta doğrulama gerekli';
  if (durumMetin) {
    durumMetin.textContent = dogrulanmis
      ? 'Hesabınıza güvenli şekilde bağlısınız.'
      : 'Premium ve eklenti için e-postanızı doğrulayın.';
  }
  profilCkysDuzeltmeGorunumunuAyarla(veri);
  profilGorunumuAyarla(true, adminMi(veri));
  maasModuluLinkleriniAyarla(maasModuluYetkiliMi(veri));
  profilEpostaDogrulamaGorunumunuAyarla();
...
async function epostaDogrulamaGonder() {
  const user = auth.currentUser;
  if (!user) {
    window.location.href = 'giris.html?sonra=profil.html';
    return;
  }
  const btn = el('btnEpostaDogrulamaGonder');
  if (btn) btn.disabled = true;
  profilEpostaDogrulamaDurumYaz('Doğrulama bağlantısı gönderiliyor...', 'bilgi');
  try {
    await sendEmailVerification(user);
    profilEpostaDogrulamaDurumYaz('Bağlantı gönderildi. Gelen kutusu ve spam klasörünü kontrol edin.', 'basari');
  } catch (err) {
    console.warn('[profil] e-posta doğrulama gönderilemedi:', err?.message || err);
    profilEpostaDogrulamaDurumYaz('Doğrulama e-postası gönderilemedi. Biraz sonra tekrar deneyin.', 'hata');
  } finally {
    if (btn) btn.disabled = false;
  }
}

async function epostaDogrulamaKontrolEt() {
  const user = auth.currentUser;
  if (!user) return;
  const btn = el('btnEpostaDogrulamaKontrol');
  if (btn) btn.disabled = true;
  profilEpostaDogrulamaDurumYaz('E-posta durumu kontrol ediliyor...', 'bilgi');
  try {
    await reload(user);
    if (auth.currentUser?.emailVerified === true) {
      await auth.currentUser.getIdToken(true).catch(() => null);
      profilEpostaDogrulamaDurumYaz('E-posta doğrulandı. Eklentiye giriş yapabilirsiniz.', 'basari');
      profilEpostaDogrulamaGorunumunuAyarla();
      profiliYansit(Object.assign({}, _sonProfilVerisi, {
        email: auth.currentUser.email,
        displayName: auth.currentUser.displayName || _sonProfilVerisi.displayName
      }));
      return;
    }
    profilEpostaDogrulamaDurumYaz('Henüz doğrulanmadı. Maildeki bağlantıya tıklayıp tekrar kontrol edin.', 'hata');
  } catch (err) {
    console.warn('[profil] e-posta doğrulama kontrol hatası:', err?.message || err);
    profilEpostaDogrulamaDurumYaz('Doğrulama durumu kontrol edilemedi.', 'hata');
  } finally {
    if (btn) btn.disabled = false;
  }
}
...
function olaylariBagla() {
  ...
  el('btnEpostaDogrulamaGonder')?.addEventListener('click', epostaDogrulamaGonder);
  el('btnEpostaDogrulamaKontrol')?.addEventListener('click', epostaDogrulamaKontrolEt);
}

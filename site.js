...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function _hypV2SiteVeriUygula(resp) {
    let sayac = 0;
    if (resp.ayAnahtari && typeof window.hesapAktifAyAyarla === 'function') {
      try { window.hesapAktifAyAyarla(resp.ayAnahtari, { kaynak: 'eklenti', yuklemeSirasi: true }); } catch (_) {}
    }
    ...
    try { window.dispatchEvent(new CustomEvent('hesap-verisi-degisti', { detail: { kaynak: 'site', ayAnahtari: resp.ayAnahtari || '' } })); } catch (_) {}
    return sayac;
  }

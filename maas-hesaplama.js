(function () {
  ...
  const KAYIT_SURUM = '2026-06-maas-ocak-piyasa-tahmini';
  ...
  const HYP2_KRITER_PREFIXLERI = [
    'htTarama', 'htTakip', 'dmTarama', 'dmTakip', 'obzTarama', 'obzTakip',
    'serviks', 'kolorektal', 'meme', 'kvrTarama', 'kvrTakip', 'yasli',
    'kah', 'inme', 'kbh', 'koah', 'astim', 'otizm'
  ];
  const HYP2_KATSAYI_TABLOSU = {
  ...
  const HYP2_SUREC_KATSAYI = { asgari: 50, azami: 80, belowMin: 1, maxCoeff: 1 };
  ...
  function varsayilanHesapYili() {
    return new Date().getFullYear();
  }
  function ocakZamHedefYili() {
    return varsayilanHesapYili() + 1;
  }
  function hesapYilSecenekleriniHazirla(seciliYil = null) {
    const select = form?.elements?.hesapYil;
    if (!select) return;
    const simdikiYil = varsayilanHesapYili();
    const hedefYil = Math.max(simdikiYil, Math.floor(Number(seciliYil || select.value) || simdikiYil));
    select.innerHTML = '';
    [simdikiYil, simdikiYil + 1].forEach((yil) => {
      const option = document.createElement('option');
      option.value = String(yil);
      option.textContent = String(yil);
      select.appendChild(option);
    });
    select.value = Array.from(select.options).some((option) => option.value === String(hedefYil))
      ? String(hedefYil)
      : String(simdikiYil);
  }
  ...
  function hesapDonemiOku(veri) {
    const hamAy = String(veri.get('hesapAy') || '').trim();
    const yilAy = hamAy.match(/^(\d{4})-(\d{1,2})$/);
    if (yilAy) {
      return {
        yil: Math.max(2000, Math.floor(Number(yilAy[1]) || varsayilanHesapYili())),
        ay: kisitla(Math.floor(Number(yilAy[2]) || 1), 1, 12)
      };
    }
    const yil = Math.max(2000, Math.floor(sayi(veri, 'hesapYil') || varsayilanHesapYili()));
    const ay = kisitla(Math.floor(Number(hamAy.replace(',', '.')) || 1), 1, 12);
    return { yil, ay };
  }

  function hesapAyDegeriniYilaBagla(deger, yil) {
    const ham = String(deger || '').trim();
    if (/^\d{4}-\d{1,2}$/.test(ham)) return ham;
    const ay = kisitla(Math.floor(Number(ham.replace(',', '.')) || 1), 1, 12);
    const hedefYil = Math.max(2000, Math.floor(Number(yil) || varsayilanHesapYili()));
    return `${hedefYil}-${ay}`;
  }

  function hesapDonemiYiliniFormaYansit() {
    if (!form?.elements?.hesapYil) return;
    const donem = hesapDonemiOku(new FormData(form));
    form.elements.hesapYil.value = String(donem.yil);
  }

  const MAAS_AY_ADLARI = ['Ocak', 'Şubat', 'Mart', 'Nisan', 'Mayıs', 'Haziran', 'Temmuz', 'Ağustos', 'Eylül', 'Ekim', 'Kasım', 'Aralık'];
  const MAAS_ODEME_AYLARI = ['Şubatta', 'Martta', 'Nisanda', 'Mayısta', 'Haziranda', 'Temmuzda', 'Ağustosta', 'Eylülde', 'Ekimde', 'Kasımda', 'Aralıkta', 'Ocakta'];

  function hesapAySecenekleriniHazirla() {
    const select = form?.elements?.hesapAy;
    if (!select) return;
    const simdikiYil = varsayilanHesapYili();
    const donem = hesapDonemiOku(new FormData(form));
    const seciliYil = Math.max(
      simdikiYil,
      Math.floor(Number(form?.elements?.hesapYil?.value) || donem.yil || simdikiYil)
    );
    hesapYilSecenekleriniHazirla(seciliYil);
    const secilecekAy = kisitla(Math.floor(Number(donem.ay) || 5), 1, 12);
    select.innerHTML = '';
    MAAS_AY_ADLARI.forEach((ayAdi, indeks) => {
      const ay = indeks + 1;
      const option = document.createElement('option');
      option.value = `${seciliYil}-${ay}`;
      option.textContent = `${ayAdi} ${seciliYil} (${MAAS_ODEME_AYLARI[indeks]} alınacak maaş)`;
      select.appendChild(option);
    });
    select.value = `${seciliYil}-${secilecekAy}`;
    hesapDonemiYiliniFormaYansit();
  }

  const ayYilAnahtari = (veri) => {
    const { yil, ay } = hesapDonemiOku(veri);
    return `${yil}-${ay}`;
  };
  ...
  function senaryoZamOrani(veri, senaryoAdi, oranAdi) {
    const senaryo = String(veri.get(senaryoAdi) || '0');
    if (senaryo === 'manuel') return Math.max(0, sayi(veri, oranAdi));
    const oran = Number(senaryo);
    return Number.isFinite(oran) ? Math.max(0, oran) : 0;
  }

  function secilenOcakZamOrani(veri) {
    return senaryoZamOrani(veri, 'ocakZamSenaryosu', 'ocakZamOrani');
  }

  function secilenTemmuzZamOrani(veri) {
    return senaryoZamOrani(veri, 'temmuzZamSenaryosu', 'temmuzZamOrani');
  }

  function etkiliZamOrani(veri) {
    const donem = hesapDonemiOku(veri);
    const ocakZam = donem.yil >= ocakZamHedefYili() ? secilenOcakZamOrani(veri) : 0;
    const temmuzZamAktif = donem.yil > varsayilanHesapYili() || donem.ay >= 7;
    const temmuzZam = temmuzZamAktif ? secilenTemmuzZamOrani(veri) : 0;
    const ocakCarpani = zamCarpani(ocakZam);
    const temmuzCarpani = zamCarpani(temmuzZam);
    return Math.max(0, (ocakCarpani * temmuzCarpani - 1) * 100);
  }

  function tahminiZamOrani(veri) {
    return etkiliZamOrani(veri);
  }

  function elleZamSenaryosunuUygula(donem = 'temmuz') {
    if (!form) return;
    const oranEl = form.elements?.[`${donem}ZamOrani`];
    const senaryoEl = form.elements?.[`${donem}ZamSenaryosu`];
    if (!oranEl || !senaryoEl) return;
    const ham = String(oranEl.value || '').trim();
    if (ham !== '' && senaryoEl.value !== 'manuel') senaryoEl.value = 'manuel';
    elleZamAlaniGorunumunuUygula(donem);
  }

  function elleZamAlaniGorunumunuUygula(donem = 'temmuz') {
    if (!form) return;
    const alan = form.querySelector(`[data-elle-zam-alani="${donem}"]`);
    const senaryoEl = form.elements?.[`${donem}ZamSenaryosu`];
    if (!alan || !senaryoEl) return;
    alan.hidden = String(senaryoEl.value || '') !== 'manuel';
  }

  function tumElleZamAlanlariniUygula() {
    elleZamSenaryosunuUygula('ocak');
    elleZamSenaryosunuUygula('temmuz');
    elleZamAlaniGorunumunuUygula('ocak');
    elleZamAlaniGorunumunuUygula('temmuz');
  }
  ...
  function formVerisiniUygula(veri, secenekler = {}) {
    ...
    const kayitliHesapYil = Math.max(2000, Math.floor(Number(veri.hesapYil) || varsayilanHesapYili()));
    const kayitliHesapAy = Object.prototype.hasOwnProperty.call(veri, 'hesapAy')
      ? hesapAyDegeriniYilaBagla(veri.hesapAy, kayitliHesapYil)
      : `${kayitliHesapYil}-5`;
    const kayitliIlce = Object.prototype.hasOwnProperty.call(veri, 'calisilanIlce')
      ? veri.calisilanIlce
      : null;
    kayitYukleniyor = true;
    if (form.elements?.hesapYil) {
      hesapYilSecenekleriniHazirla(kayitliHesapYil);
      form.elements.hesapYil.value = String(kayitliHesapYil);
    }
    hesapAySecenekleriniHazirla();
    Object.entries(veri).forEach(([ad, deger]) => {
      ...
      if (ad === 'hesapAy' || ad === 'hesapYil') return;
    ...
    if (form.elements?.hesapYil) form.elements.hesapYil.value = String(kayitliHesapYil);
    if (form.elements?.hesapAy) form.elements.hesapAy.value = kayitliHesapAy;
    hesapDonemiYiliniFormaYansit();
  ...
  function maasBulutAksiyonlariniGuncelle(uid = window._currentUid) {
    const girisVar = !!String(uid || '').trim();
    const kaydetBtn = document.getElementById('maasBilgileriKaydet');
    const cekBtn = document.getElementById('maasBuluttanCek');
    const girisCta = document.getElementById('maasGirisKaydetCta');
    const durumEl = document.getElementById('maasKayitDurumu');
    if (kaydetBtn) kaydetBtn.hidden = !girisVar;
    if (cekBtn) cekBtn.hidden = !girisVar;
    if (girisCta) girisCta.hidden = girisVar;
    if (durumEl) durumEl.hidden = !girisVar;
  }

  function detayliNufusSummaryUyarisiGuncelle(eksik) {
    const uyari = document.getElementById('maasDetayNufusUyari');
    const detayKutu = form.querySelector('.maas-nufus-detay-popup');
    const eksikMi = !!eksik;
    if (uyari) uyari.hidden = !eksikMi;
    if (detayKutu) detayKutu.classList.toggle('maas-detay-eksik', eksikMi);
  }
  ...
  function varsayilanGecisleriniUygula() {
    try {
      ...
      const ocakZamSenaryosuEl = form.elements?.ocakZamSenaryosu;
      if (ocakZamSenaryosuEl) {
        if (!ocakZamSenaryosuEl.value || ocakZamSenaryosuEl.value === '10') ocakZamSenaryosuEl.value = '7';
        if (ocakZamSenaryosuEl.value === '6') ocakZamSenaryosuEl.value = '5';
      }
      const temmuzZamSenaryosuEl = form.elements?.temmuzZamSenaryosu;
      if (temmuzZamSenaryosuEl && (!temmuzZamSenaryosuEl.value || temmuzZamSenaryosuEl.value === '0' || temmuzZamSenaryosuEl.value === '13.75')) {
        temmuzZamSenaryosuEl.value = '13.96';
      }
  ...
  function ayItibariyleKumulatifMatrahHesapla(veri, aylikMatrah, oncekiAylarMatrahi) {
    const { ay: hesapAy } = hesapDonemiOku(veri);
  ...
  function otomatikKumulatifMatrahHesapla(veri, aylikMatrah, otomatikMatrah, oncekiAylarMatrahi) {
    ...
    const sonrakiYilMi = donem.yil > varsayilanHesapYili();
    const secilenOcakZam = donem.yil >= ocakZamHedefYili() ? secilenOcakZamOrani(veri) : 0;
    const secilenTemmuzZam = secilenTemmuzZamOrani(veri);
    const ayZamliMi = ay >= 7;
    const matrahBaz = Math.max(0, oncekiAylarMatrahi || otomatikMatrah || aylikMatrah);
    const vergiMatrahiIcinTemmuzCarpani = ayZamliMi ? zamCarpani(secilenTemmuzZam) : 1;
    const vergiMatrahiIcinOcakCarpani = sonrakiYilMi ? zamCarpani(secilenOcakZam) : 1;
    const bazMatrah = Math.max(0, matrahBaz / (vergiMatrahiIcinTemmuzCarpani * vergiMatrahiIcinOcakCarpani));
    const ocakSonrasiMatrah = sonrakiYilMi ? bazMatrah * vergiMatrahiIcinOcakCarpani : bazMatrah;
    const temmuzSonrasiMatrah = ayZamliMi ? ocakSonrasiMatrah * vergiMatrahiIcinTemmuzCarpani : ocakSonrasiMatrah;
    const ocakBaslangicMatrahi = bazMatrah * 0.85;
    if (ay === 1) return Math.max(0, ocakBaslangicMatrahi);
    if (ay <= 7) return Math.max(0, ocakBaslangicMatrahi + ((ay - 1) * ocakSonrasiMatrah));
    return Math.max(0, ocakBaslangicMatrahi + (6 * ocakSonrasiMatrah) + ((ay - 7) * temmuzSonrasiMatrah));
  }
  ...
  function sgkMatrahiHesapla(veri, zamOrani) {
    ...
    const { yil: hesapYil, ay: hesapAy } = hesapDonemiOku(veri);
  ...
  function izinKesintiHesapla(veri, izinKesintiMatrahi) {
    const durumRaw = String(veri.get('izinDurumu') || 'yok');
    const izinAyGunSayisi = ayGunSayisi(veri);
    // SAHU secenekleri vekaletsiz izinden farkli: matrahin %30'u kesilir (vekaletsizde %50).
    // Gun her zaman ayin 15'ine gore: gidiste 15. gun dahil ay sonuna kadar, donuste ilk 14 gun.
    const sahuTipi = durumRaw === 'sahuGidis' || durumRaw === 'sahuDonus' || durumRaw === 'sahuTumAy';
    let sahuGun = null;
    if (durumRaw === 'sahuTumAy') {
      sahuGun = izinAyGunSayisi;
    } else if (durumRaw === 'sahuGidis') {
      sahuGun = izinAyGunSayisi - 15 + 1;
    } else if (durumRaw === 'sahuDonus') {
      sahuGun = 14;
    }
    const durum = sahuTipi ? 'sahu' : durumRaw;
    const gun = Math.min(izinAyGunSayisi, sahuGun != null ? Math.max(0, sahuGun) : Math.max(0, sayi(veri, 'izinGun')));
    const manuel = sayi(veri, 'izinRaporKesinti');
    const izinKesintiOrani = durum === 'sahu' ? 0.3 : (durum === 'vekaletsiz' ? 0.5 : (durum === 'rapor' ? 0.5 : 0));
  ...
  function hesapla(secenekler = {}) {
    ...
    const ocakOncekiAralikMatrahi = hesapDonemiOku(veri).ay === 1 && !String(veri.get('kumulatifMatrah') || '').trim() && !ayItibariyleKumulatifMatrah;
  ...
  function hypKatsayiZorunluDurumunuGuncelle() {
    if (!form) return;
    const hypEl = form.elements?.hypKatsayi;
    const alan = hypEl?.closest?.('.maas-hyp-katsayi-alani');
    if (!hypEl || !alan) return;
    const ham = String(hypEl.value || '').replace(',', '.').trim();
    const deger = Number(ham);
    const eksik = ham === '' || !Number.isFinite(deger) || deger <= 0;
    alan.classList.toggle('maas-hyp-katsayi-eksik', eksik);
    hypEl.setAttribute('aria-invalid', eksik ? 'true' : 'false');
  }

  function tesvikUyariDurumunuGuncelle() {
    if (!form) return;
    const sayisalDeger = (el) => {
      const ham = String(el?.value || '').replace(',', '.').trim();
      if (!ham) return NaN;
      return Number(ham);
    };
    const ayarla = (el, eksik) => {
      const alan = el?.closest?.('.maas-tesvik-uyarili');
      const uyari = alan?.querySelector('.maas-tesvik-uyari');
      if (!el || !alan || !uyari) return;
      uyari.hidden = !eksik;
      alan.classList.toggle('maas-tesvik-eksik', eksik);
      el.setAttribute('aria-invalid', eksik ? 'true' : 'false');
    };

    const hypEl = form.elements?.hypKatsayi;
    const otoEl = form.elements?.hypKatsayiCanliCek;
    const hypDeger = sayisalDeger(hypEl);
    const girisVar = Boolean(String(window._currentUid || '').trim());
    ayarla(hypEl, !girisVar || !otoEl?.checked || !Number.isFinite(hypDeger) || hypDeger <= 0);

    ['khtOrani', 'basvuruOrani', 'akilciIlacOrani', 'memnuniyetOrani'].forEach((ad) => {
      const el = form.elements?.[ad];
      const deger = sayisalDeger(el);
      ayarla(el, !Number.isFinite(deger) || deger <= 0);
    });
  }

  function muayeneUyariDurumunuGuncelle() {
    if (!form) return;
    const adlar = ['muayeneHedef', 'muayeneOrtalama'];
    adlar.forEach((ad) => {
      const input = form.elements?.[ad];
      const alan = input?.closest?.('.maas-tesvik-muayene');
      const uyari = alan?.querySelector('.maas-muayene-uyari');
      if (!input || !alan || !uyari) return;
      const ham = String(input.value || '').replace(',', '.').trim();
      const deger = Number(ham);
      const eksik = ham === '' || !Number.isFinite(deger) || deger <= 0;
      uyari.hidden = !eksik;
      alan.classList.toggle('maas-muayene-eksik', eksik);
      input.setAttribute('aria-invalid', eksik ? 'true' : 'false');
    });
  }

  function manuelNufusUyariDurumunuGuncelle() {
    if (!form) return;
    const manuelMod = String(form.elements?.nufusPuaniModu?.value || 'manuel') === 'manuel';
    const adlar = ['manuelToplamNufus', 'hamNufusPuani', 'gelmeyenNufusPuani'];
    adlar.forEach((ad) => {
      const input = form.elements?.[ad];
      const alan = input?.closest?.('.maas-manuel-nufus-alani');
      const uyari = alan?.querySelector('.maas-nufus-uyari');
      if (!input || !alan || !uyari) return;
      const ham = String(input.value || '').replace(',', '.').trim();
      const eksik = manuelMod && ham === '';
      uyari.hidden = !eksik;
      alan.classList.toggle('maas-manuel-nufus-eksik', eksik);
      input.setAttribute('aria-invalid', eksik ? 'true' : 'false');
    });
  }
  ...
  function render() {
    hesabaEsasNufusPuaniAlaniniGuncelle();
    const h = hesapla();
    hypKatsayiZorunluDurumunuGuncelle();
    tesvikUyariDurumunuGuncelle();
    manuelNufusUyariDurumunuGuncelle();
    muayeneUyariDurumunuGuncelle();
    try {
    ...
    detayliNufusSummaryUyarisiGuncelle(h.nufus.mod !== 'manuel' && h.nufus.detayZorunluEksik);
        ...
        <div class="maas-tablo-kutu">
          ...
          <table>
            <tbody>
              ...
              ${satir('İzin/rapor kesintisi', -h.izinRaporKesinti, h.izinDetay.manuel > 0 ? 'Manuel girildi' : (h.izinDetay.durum === 'sahu' ? `SAHU: temel matrahın %30'u; ${h.izinDetay.gun.toLocaleString('tr-TR')}/${h.izinDetay.izinAyGunSayisi} gün; matrah ${para(h.izinKesintiMatrahi)}` : (h.izinDetay.durum === 'vekaletsiz' ? `Vekaletsiz izin: temel matrahın %50'si; ${h.izinDetay.gun.toLocaleString('tr-TR')}/${h.izinDetay.izinAyGunSayisi} gün; matrah ${para(h.izinKesintiMatrahi)}` : (h.izinDetay.durum === 'rapor' ? `Rapor: temel matrahın %50'si; ${h.izinDetay.gun.toLocaleString('tr-TR')}/${h.izinDetay.izinAyGunSayisi} gün; matrah ${para(h.izinKesintiMatrahi)}` : `${h.izinDetay.gun.toLocaleString('tr-TR')}/${h.izinDetay.izinAyGunSayisi} gün; ${h.izinDetay.durum}; matrah ${para(h.izinKesintiMatrahi)}`))))}
  ...
  function nufusPuaniModuGorunumunuUygula() {
    ...
    if (manuelMod) detayliNufusSummaryUyarisiGuncelle(false);
    manuelNufusUyariDurumunuGuncelle();
  }
  ...
  function hypKatsayiTablodan(oran, tablo) {
    if (!tablo) return 1;
    if (oran < tablo.asgari) return tablo.belowMin;
    if (oran >= tablo.azami) return tablo.maxCoeff;
    return 1 + (tablo.maxCoeff - 1) * (oran - tablo.asgari) / (tablo.azami - tablo.asgari);
  }

  function hypBasariKSKriterlerden(kriterler) {
    if (!kriterler || typeof kriterler !== 'object') return 0;
    let katsayi = 1;
    let veriVar = false;
    HYP2_KRITER_PREFIXLERI.forEach((prefix) => {
      const tablo = HYP2_KATSAYI_TABLOSU[prefix];
      const kayit = kriterler[prefix] || {};
      const gereken = Math.max(0, Number(kayit.g || kayit.G || 0));
      const yapilan = Math.max(0, Number(kayit.y || kayit.Y || 0));
      const devirHam = Math.max(0, Number(kayit.d || kayit.D || 0));
      if (gereken > 0) veriVar = true;
      const devir = gereken > 0 && yapilan >= gereken * 0.10 ? devirHam : 0;
      const oran = gereken > 0 ? ((yapilan + devir) / gereken) * 100 : 0;
      katsayi *= hypKatsayiTablodan(oran, tablo);
    });
    if (!veriVar) return 0;
    katsayi *= hypKatsayiTablodan(80, HYP2_SUREC_KATSAYI);
    return katsayi;
  }

  function sonHypBasariKSOnbelleginiOku() {
    try {
      const deger = Number(String(localStorage.getItem('hypBasariHamKS') || '').replace(',', '.'));
  ...
  function hypMaasaEsasKatsayiBul(kayit) {
    ...
    let hamHypKs = hypBasariKSKriterlerden(kayit?.hypV2?.kriterler);
    if (!(hamHypKs > 0)) hamHypKs = sonHypBasariKSOnbelleginiOku();
    if (!Number.isFinite(hamHypKs) || hamHypKs <= 0) return null;
    const nufus = Number(kayit?.hypV2?.nufus || 0);
    const nufusTip = String(kayit?.hypV2?.nufusTip || '3500');
    if (!Number.isFinite(nufus) || nufus <= 0) return kisitla(hamHypKs, 0.9, 2);
    const dusukTip = nufusTip === 'dusuk' || nufusTip === '2400';
    const cap = dusukTip ? 2400 : 3500;
    const baz = dusukTip ? 2400 : 4000;
    const eff = Math.min(nufus, cap);
    const tavanKs = Math.min(baz / Math.max(1, eff), 1.5);
    return kisitla(Math.min(hamHypKs, tavanKs), 0.9, 2);
  }
  ...
  form.addEventListener('input', (e) => {
    ...
    if (e.target?.name === 'ocakZamOrani' || e.target?.name === 'temmuzZamOrani') {
      elleZamSenaryosunuUygula(e.target.name === 'ocakZamOrani' ? 'ocak' : 'temmuz');
      sabitEkGelirleriUygula();
      sendikaOdenegiUygula();
      sendikaDurumunuUygula();
    }
  ...
  function izinDurumuSahuUygula(deger) {
    const gunInput = form.querySelector('[name="izinGun"]');
    if (!gunInput) return;
    // SAHU secenekleri sabit ayin 15'ine gore; gun otomatik, kullanici girmez.
    if (deger === 'sahuGidis' || deger === 'sahuDonus' || deger === 'sahuTumAy') {
      const ayGun = ayGunSayisi(new FormData(form));
      gunInput.value = String(deger === 'sahuGidis' ? Math.max(0, ayGun - 15 + 1) : (deger === 'sahuDonus' ? 14 : ayGun));
      gunInput.readOnly = true;
    } else if (gunInput.readOnly) {
      gunInput.readOnly = false;
    }
  }

  form.addEventListener('change', (e) => {
    if (e.target?.name === 'izinDurumu') izinDurumuSahuUygula(e.target.value);
    if (e.target?.name === 'hesapYil') hesapAySecenekleriniHazirla();
    if (e.target?.name === 'hesapAy' || e.target?.name === 'hesapYil') {
      hesapDonemiYiliniFormaYansit();
      izinDurumuSahuUygula(form.querySelector('[name="izinDurumu"]')?.value);
    }
    ...
    if (e.target?.name === 'ocakZamSenaryosu' || e.target?.name === 'temmuzZamSenaryosu') {
      elleZamAlaniGorunumunuUygula(e.target.name === 'ocakZamSenaryosu' ? 'ocak' : 'temmuz');
    }
    if (['hesapAy', 'hesapYil', 'kadroDurumu', 'ocakZamSenaryosu', 'ocakZamOrani', 'temmuzZamSenaryosu', 'temmuzZamOrani'].includes(e.target?.name)) sabitEkGelirleriUygula();
    if (['hesapAy', 'hesapYil', 'sendikaUyeligi', 'ocakZamSenaryosu', 'ocakZamOrani', 'temmuzZamSenaryosu', 'temmuzZamOrani'].includes(e.target?.name)) {
      sendikaOdenegiUygula();
      sendikaDurumunuUygula();
    }
    yerelKayitPlanla();
  ...
  adminKilitHazirla();
  maasBulutAksiyonlariniGuncelle();
  hesapYilSecenekleriniHazirla();
  hesapAySecenekleriniHazirla();
  yerelKayitYukle();
  hesapDonemiYiliniFormaYansit();

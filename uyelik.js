...
function denemeBasariYaz(sonuc) {
  ...
  durumYaz(`✓ Premium deneme başladı.${bitis ? ` Bitiş tarihi: ${bitis}.` : ''} Profilinize yönlendiriliyorsunuz...`, 'basari');
  if (gonderBtn) {
...
const ckysDuzeltmeForm = document.getElementById('ckysDuzeltmeForm');
const ckysDuzeltmeDurum = document.getElementById('ckysDuzeltmeDurum');

function ckysDuzeltmeDurumYaz(metin, tip) {
  if (!ckysDuzeltmeDurum) return;
  ckysDuzeltmeDurum.textContent = metin || '';
  ckysDuzeltmeDurum.className = 'form-durum';
  if (tip) ckysDuzeltmeDurum.classList.add(tip);
}

function ckysDuzeltmeMailOlustur(ckysKodu) {
  const user = auth.currentUser || window._currentUser || null;
  const email = user?.email || '';
  const uid = user?.uid || '';
  const subject = 'Ahek Plus ÇKYS kodu düzeltme talebi';
  const body = [
    'Merhaba,',
    '',
    'Ahek Plus üyeliğimde ÇKYS kodumu yanlış yazdım. Manuel düzeltme rica ederim.',
    '',
    `Doğru ÇKYS kodu: ${ckysKodu}`,
    `Hesap e-postası: ${email || 'Giriş bilgisi alınamadı'}`,
    `Firebase kullanıcı ID: ${uid || 'Ulaşılamadı'}`,
    '',
    'Teşekkürler.',
  ].join('\n');
  return `mailto:ahekplus@gmail.com?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
}

ckysDuzeltmeForm?.addEventListener('submit', (e) => {
  e.preventDefault();
  const fd = new FormData(ckysDuzeltmeForm);
  const ckysKodu = String(fd.get('ckysDuzeltmeKodu') || '').replace(/\D+/g, '');
  if (ckysKodu.length < 3 || ckysKodu.length > 20) {
    ckysDuzeltmeDurumYaz('Geçerli doğru ÇKYS kodunu girin.', 'hata');
    ckysDuzeltmeForm.querySelector('input[name="ckysDuzeltmeKodu"]')?.focus();
    return;
  }
  window.location.href = ckysDuzeltmeMailOlustur(ckysKodu);
  ckysDuzeltmeDurumYaz('E-posta uygulamanız açıldı. Metinde e-posta, kullanıcı id ve doğru ÇKYS kodu hazırlandı.', 'bilgi');
});

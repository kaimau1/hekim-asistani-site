...
function callableHataMesaji(error) {
  ...
  if (durumKodu.includes('failed-precondition') && /eposta|e-posta|dogrula|doğrula/i.test(mesaj)) {
    return 'Premium deneme için önce e-posta adresinizi doğrulayın. Doğrulamadan sonra tekrar deneyin.';
  }
...
async function premiumDenemeBaslat(ckysKodu) {
  ...
  if (await epostaDogrulamaGerekliMi(user)) {
    throw new Error('Premium deneme için önce e-posta adresinizi doğrulayın. Doğrulama bağlantısını tekrar gönderdik; gelen kutunuzu ve spam klasörünü kontrol edin.');
  }

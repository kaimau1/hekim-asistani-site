...
(function() {
  ...
  function tumunuSifirla(tabloAd) {
    if (!confirm('Tüm değerleri sıfırlamak istediğine emin misin?\n(Kriterler + profil + hedefler temizlenir, ŞABLONLAR korunur)')) return;
    const veriAttr = _veriSecAttr(tabloAd);
    const profilAttr = _profilSecAttr(tabloAd);
    _krIcin(tabloAd).forEach(k => {
      ['G','Y','D','H'].forEach(s => {
        const e = document.querySelector(`input[${veriAttr}="${k.prefix}-${s}"]`);
        if (e) e.value = '';
      });
      const maxEl = document.querySelector(`input[${profilAttr}="${k.prefix}-max"]`);
      const eforEl = document.querySelector(`select[${profilAttr}="${k.prefix}-efor"]`);
      const oncelikEl = document.querySelector(`select[${profilAttr}="${k.prefix}-oncelik"]`);
      if (maxEl) maxEl.value = '';
      if (eforEl) eforEl.value = '3';
      if (oncelikEl) { oncelikEl.value = '5'; oncelikEl.hidden = true; }
    });
    if (tabloAd === 'hypv2') {
      const n = document.getElementById('hypV2Nufus'); if (n) n.value = '';
      if (window.hypV2Hesapla) window.hypV2Hesapla();
    } else {
      const a = document.getElementById('ascHekimKS'); if (a) a.value = '';
      if (window.ascHesapla) window.ascHesapla();
    }
    try { window.dispatchEvent(new CustomEvent('hesap-verisi-degisti')); } catch (_) {}
    if (window.siteToast) window.siteToast('🗑 Tüm değerler sıfırlandı', 'uyari');
  }

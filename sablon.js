...
(function() {
  ...
  const GENEL_SABLONLAR = {
    hypv2: {
      '🌐 Genel — Optimal': {
        htTarama:   { max: 100, efor: 2, oncelik: 5 },
        htTakip:    { max: 100, efor: 4, oncelik: 5 },
        dmTarama:   { max: 100, efor: 1, oncelik: 5 },
        dmTakip:    { max: 100, efor: 3, oncelik: 5 },
        obzTarama:  { max: 100, efor: 2, oncelik: 5 },
        obzTakip:   { max: 100, efor: 3, oncelik: 5 },
        serviks:    { max: 100, efor: 6, oncelik: 2 },
        kolorektal: { max: 100, efor: 6, oncelik: 1 },
        meme:       { max: 100, efor: 6, oncelik: 3 },
        kvrTarama:  { max: 100, efor: 3, oncelik: 5 },
        kvrTakip:   { max: 100, efor: 1, oncelik: 5 },
        yasli:      { max: 100, efor: 2, oncelik: 5 },
        kah:        { max: 100, efor: 2, oncelik: 5 },
        inme:       { max: 100, efor: 4, oncelik: 5 },
        kbh:        { max: 100, efor: 3, oncelik: 5 },
        koah:       { max: 100, efor: 5, oncelik: 5 },
        astim:      { max: 100, efor: 5, oncelik: 5 },
        otizm:      { max: 100, efor: 1, oncelik: 5 },
      },
    },
    asc: {
      '🌐 Genel — Dengeli': {
  ...
  function _yenidenHesapla(tabloAd) {
    if (tabloAd === 'hypv2') {
  ...
  function _profilHazirMi(tabloAd) {
    const attr = _profilSecAttr(tabloAd);
    const kriterler = _krIcin(tabloAd);
    return kriterler.length > 0 && kriterler.every(k =>
      !!document.querySelector(`input[${attr}="${k.prefix}-max"]`) &&
      !!document.querySelector(`select[${attr}="${k.prefix}-efor"]`)
    );
  }

  function _profilBosMu(tabloAd) {
    if (!_profilHazirMi(tabloAd)) return false;
    const attr = _profilSecAttr(tabloAd);
    return !_krIcin(tabloAd).some(k => {
  ...
  function _varsayilanSablon(tabloAd) {
    ...
    hedefler.forEach(t => {
      if (!_profilBosMu(t)) return;  // kullanıcının profili var — dokunma
  ...
  window.hesapSablonYaz = (s) => {
    ...
    dropdownDoldur();
    setTimeout(() => _varsayilanSablon(), 0);
  };
  ...
  window.addEventListener('hesap-formlari-hazir', () => _varsayilanSablon());

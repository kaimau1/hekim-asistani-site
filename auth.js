...
async function turnstileRenderEt() {
  const form = document.getElementById('authKayitForm');
  const alan = form?.querySelector('[data-turnstile-kayit]');
  if (!form || !alan || _turnstileWidgetId !== null) return;
  try {
    const turnstile = await turnstileYukle();
    _turnstileWidgetId = turnstile.render(alan, {
    ...
    durumGoster('Bot doğrulaması yüklenemedi. Lütfen sayfayı yenileyip tekrar deneyin.', 'hata');
  }
}

function turnstileTokenAl() {
  if (!turnstileYapilandirildiMi()) return '';
  if (_turnstileWidgetId !== null && window.turnstile?.getResponse) {
    return window.turnstile.getResponse(_turnstileWidgetId) || '';
  }
  return document.querySelector('#authKayitForm [name="cf-turnstile-response"]')?.value || '';
}
...
function _hypBilgiNotuGuncelle(user) {
  ...
  if (user) {
    ...
    metin.textContent = `✅ Giriş yapıldı: ${email}. Değerleriniz bu cihazda korunur ve hesabınızla güvenle senkronize edilir.`;
    hypBilgiNotu.appendChild(metin);

    const senkMetni = _sonSenkMetni || '☁ Hesap verisi yükleniyor…';
    ...
    senkRozeti.textContent = senkMetni;
    senkRozeti.title = senkMetni;
  ...
  hypBilgiNotu.appendChild(document.createTextNode(' tüm cihazlarınız arasında otomatik senkronize olur.'));
}
...
function sekmeGec(sekme) {
  ...
  if (sekme === 'kayit') setTimeout(() => { turnstileRenderEt(); }, 60);
}
...
function _webAdminYuzeyleriAyarla(admin, durum = 'hazir') {
...
function _webAdminYuzeyleriniDinle(user) {
  ...
  try {
    _webAdminYuzeyUnsub = onSnapshot(doc(db, 'users', user.uid), (snap) => {
      const veri = snap.data() || {};
      _webAdminYuzeyleriAyarla(String(veri.role || '').toLowerCase() === 'admin', 'hazir');
    }, () => {
...
async function kayitOl(ad, email, sifre) {
  ...
  try {
    await turnstileRenderEt();
    const turnstileToken = turnstileTokenAl();
    if (!turnstileToken) {
      durumGoster('Lütfen bot doğrulamasını tamamlayın.', 'hata');
      return;
    }
    _sonGirisIstegiZamani = Date.now();
...
async function sifreSifirla(email) {
  ...
  durumGoster('Güvenli şifre sıfırlama bağlantısı hazırlanıyor…', 'info');
  try {
    await sendPasswordResetEmail(auth, email);
    sekmeGec('giris');
    durumGoster('Adres kayıtlıysa şifre sıfırlama bağlantısı gönderildi. Gelen kutunuzu ve spam/gereksiz klasörünü kontrol edin.', 'ok');
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('[data-sifre-sifirla]').forEach(b =>
    b.addEventListener('click', () => {
      sekmeGec('sifirla');
      durumGoster('Hesabınıza bağlı e-posta adresini girin; kayıtlıysa güvenli bağlantı gönderilir.', 'info');
  ...
  <div id="authModal" class="modal" hidden>
    ...
    <div class="modal-icerik" role="dialog" aria-labelledby="authBaslik" aria-modal="true">
      ...
      <form class="auth-form" id="authKayitForm" data-form="kayit" novalidate>
        ...
        <div class="auth-turnstile" data-turnstile-kayit aria-label="Bot doğrulaması"></div>
      ...
      <form class="auth-form" id="authSifirlaForm" data-form="sifirla" novalidate>
        <p class="auth-bilgi">Ahek Plus hesabınıza bağlı e-posta adresinizi girin. Adres kayıtlıysa güvenli şifre sıfırlama bağlantısı gönderilir.<br><small style="color:#64748b">Bağlantı kısa süre geçerlidir. Gelen kutunuzda yoksa spam/gereksiz klasörünü kontrol edin.</small></p>

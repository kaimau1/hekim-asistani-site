// ═══ Firebase Authentication — Hekim Plus Tanıtım Sitesi ═══
...
async function _profilYukle(uid) {
  try {
    ...
    _snapshotAbort = onSnapshot(ref, (snap) => {
      ...
      // (1sn sonra tekrar okuyacak zaten; veya bir sonraki snapshot'ta).
...
function _profilKaydetDebounced() {
  if (!_aktifUid) return;
...
function _authSayfaArtiklariniTemizle() {
  ...
  document.querySelectorAll('.giris-sol, .giris-gorsel-kart, #girisBasariKarti, #authDurum').forEach((node) => {
    node.remove();
  });
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('btnAuthGiris')?.addEventListener('click', () => {
  ...
  document.querySelectorAll('[data-sifre-sifirla]').forEach(b =>
    b.addEventListener('click', () => sekmeGec('sifirla'))
  );
...
const _AUTH_ALAN_HTML = `
  <div class="auth-alan">
    <button id="btnAuthGiris" class="btn-auth-giris" type="button">Giriş Yap</button>
  ...
  <div id="authModal" class="modal" hidden>
    ...
    <div class="modal-icerik" role="dialog" aria-labelledby="authBaslik" aria-modal="true">
      ...
      <div id="authBaslik" class="auth-modal-baslik">
        ...
        <div>
          <div class="auth-modal-baslik-ana">Hekim Plus Hesabı</div>
          <div class="auth-modal-baslik-alt">Giriş yap veya yeni hesap aç</div>
        </div>
      </div>
      ...
      <form class="auth-form aktif" id="authGirisForm" data-form="giris" novalidate>
        ...
        <label class="auth-label"><span>Şifre</span><input type="password" id="girisSifre" placeholder="••••••••" required autocomplete="current-password" minlength="6"></label>
      ...
      <form class="auth-form" id="authKayitForm" data-form="kayit" novalidate>
        ...
        <label class="auth-label"><span>Şifre <em>(min. 6 karakter)</em></span><input type="password" id="kayitSifre" placeholder="••••••••" required autocomplete="new-password" minlength="6"></label>
      ...
      <form class="auth-form" id="authSifirlaForm" data-form="sifirla" novalidate>
        <p class="auth-bilgi">E-posta adresinizi girin — sıfırlama bağlantısı gönderelim.<br><small style="color:#d97706">⚠️ Mail gelmezse <b>spam/gereksiz</b> klasörünü kontrol edin.</small></p>

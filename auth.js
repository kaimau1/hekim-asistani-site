...
const rotasyonDavetKayitOlCallable = httpsCallable(functions, 'rotasyonDavetKayitOl');
...
function rotasyonDavetParametreleri() {
  const params = new URLSearchParams(location.search);
  const token = params.get('rotasyonDavet') || params.get('davet') || '';
  return token ? { group: 'ankara-tip-sahu', token } : null;
}

function rotasyonDavetModuAktifMi() {
  return !!rotasyonDavetParametreleri();
}

function googleButonlariniRotasyonDavetindeGizle() {
  if (!rotasyonDavetModuAktifMi()) return;
  // Davet kaydi artik rotasyonerlere ozel sayfada yapilir (sicil no ister). Eski linkleri yonlendir.
  if (!location.pathname.endsWith('rotasyon-kayit.html')) {
    const davet = rotasyonDavetParametreleri();
    location.replace(`rotasyon-kayit.html?davet=${encodeURIComponent(davet.token)}`);
    return;
  }
  document.querySelectorAll('[data-google-giris]').forEach((btn) => { btn.hidden = true; });
  document.querySelectorAll('.auth-ayrac').forEach((el) => { el.hidden = true; });
  document.querySelectorAll('[data-rotasyon-davet-panel]').forEach((el) => { el.hidden = false; });
  document.querySelectorAll('[data-auth-sekme="kayit"]').forEach((btn) => btn.click());
  const kayitAd = document.getElementById('kayitAd');
  if (kayitAd) {
    kayitAd.required = true;
    const label = kayitAd.closest('.auth-label');
    const em = label?.querySelector('em');
    if (em) em.textContent = '(zorunlu)';
  }
}
...
// HYP ve ASÇ artık AYRI sayfalarda; bu sayfada tablosu olmayan bölümü DOM'dan
// SIFIRLAMA — en taze yerel kayıttan koru (yoksa cloud'a boş yazıp diğer
// bölümü silerdik). Root bölüm boş gitse bile okuma tarafı ay kaydıyla
// birleştiriyor; yine de cloud dokümanını temiz tutmak için burada koruyoruz.
function _formdanTopla() {
  ...
  let yedek = {};
  try { yedek = JSON.parse(localStorage.getItem('hesap_verileri_v1') || '{}') || {}; } catch (_) {}

  let hypV2;
  if (hypVar && window.HYP2_KRITERLER) {
    const v2Kriterler = {};
    const v2Profil = {};
    ...
    hypV2 = yedek.hypV2 || { nufus: 0, nufusTip: '3500', nufusPuani: 0, kriterler: {}, profil: {} };
  }

  let asc;
  if (ascVar && window.ASC_KRITERLER) {
    const ascKriterler = {};
    const ascProfil = {};
    ...
    asc = {
      ...
      nufus: parseInt(document.getElementById('ascNufus')?.value || document.getElementById('hypV2Nufus')?.value) || 0,
      nufusTip: document.getElementById('ascNufusTip')?.value || document.getElementById('hypV2NufusTip')?.value || '3500',
      kriterler: ascKriterler,
      profil: ascProfil,
    };
  } else {
    asc = yedek.asc || { hekimKS: 0, nufus: 0, nufusTip: '3500', kriterler: {}, profil: {} };
  }
...
onAuthStateChanged(auth, (user) => {
  ...
  if (user) {
    ...
    if (!ilkDurum && Date.now() - _sonCikisIstegiZamani < 15000) {
      ...
      _hypGirisKaydetCtaGuncelle(null);
...
async function kayitOl(ad, email, sifre) {
  ...
  try {
    ...
    _sonGirisIstegiZamani = Date.now();
    if (rotasyonDavetModuAktifMi()) {
      if (!ad || ad.trim().length < 3) {
        durumGoster('Rotasyon kaydi icin ad soyad zorunlu.', 'hata');
        return;
      }
      const davet = rotasyonDavetParametreleri();
      const sonuc = await rotasyonDavetKayitOlCallable({
        ad,
        email,
        sifre,
        davetToken: davet.token,
        turnstileToken,
      });
      const customToken = sonuc?.data?.customToken;
      if (!customToken) throw new Error('Rotasyon kaydi tamamlanamadi.');
      const cred = await signInWithCustomToken(auth, customToken);
      if (cred.user) {
        try { await updateProfile(cred.user, { displayName: ad }); } catch (_) {}
        try { await sendEmailVerification(cred.user); } catch (_) {}
      }
      durumGoster('Rotasyon kaydi olusturuldu. Rotasyon ekranina yonlendiriliyorsunuz.', 'ok');
      location.href = 'rotasyon.html';
      return;
    }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  googleButonlariniRotasyonDavetindeGizle();
  ...
  document.getElementById('authKayitForm')?.addEventListener('submit', (e) => {
    e.preventDefault();
    const onay = document.getElementById('kayitSozlesmeOnay');
    if (onay && !onay.checked) {
      durumGoster('Devam etmek için Kullanım Şartları ve KVKK Aydınlatma Metni\'ni onaylayın.', 'hata');
      try { onay.focus(); } catch (_) {}
      return;
    }
  ...
  <div id="authModal" class="modal" hidden>
    ...
    <div class="modal-icerik" role="dialog" aria-labelledby="authBaslik" aria-modal="true">
      ...
      <form class="auth-form" id="authKayitForm" data-form="kayit" novalidate>
        ...
        <label class="auth-onay"><input type="checkbox" id="kayitSozlesmeOnay" required><span><b>Kullanım Şartları</b>'nı ve <b>KVKK Aydınlatma Metni</b>'ni okudum, kabul ediyorum.</span></label>

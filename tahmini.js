...
(function () {
  ...
  function yeniden() {
    ...
    const kartHtml = KRITERLER.map(k => {
      ...
      // localStorage'dan gelen değerler innerHTML'e girmeden sayıya zorlanır
      const formul = k.key === 'osbTarama' && typeof v.buAySayilacak === 'number'
        ? `OSB Ay Oku: bu ay sayılacak ${Number(v.buAySayilacak) || 0}`
        : k.ilkTakip
        ? `Hiç taranmayan ${Number(v.hicTakip) || 0} / ${k.bolen}`
        : `(Toplam ${Number(v.toplam) || 0} − Zamanında ${Number(v.zamaninda) || 0}) / ${k.bolen}`;

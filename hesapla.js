...
function _dikeyKaydiriciBul(el) {
  let aktif = el instanceof Element ? el : null;
  while (aktif && aktif !== document.body) {
    const stil = window.getComputedStyle(aktif);
    const overflowY = stil.overflowY || stil.overflow;
    const kaydirilabilir = /(auto|scroll|overlay)/.test(overflowY);
    if (kaydirilabilir && aktif.scrollHeight > aktif.clientHeight + 1) {
      return aktif;
    }
    aktif = aktif.parentElement;
  }
  return null;
}

// Mouse tekerleği sayı inputlarında değeri değiştirmesin.
// Ayrıca tablo sarmalı veya input üstündeyken iç alan kayamıyorsa,
// tekerlek hareketini ana sayfaya aktar ki preview takılı kalmasın.
document.addEventListener('wheel', (e) => {
  const hedef = e.target instanceof Element ? e.target : null;
  if (!hedef) return;
  if (document.body.classList.contains('modal-acik') || document.body.classList.contains('tur-aktif')) return;

  const sayiInput = hedef.closest('input[type="number"]');
  const tabloIci = hedef.closest('.tablo-sarmal');
  if (!sayiInput && !tabloIci) return;

  const kaydirici = _dikeyKaydiriciBul(hedef);
  const asagi = e.deltaY > 0;
  const alanDikeyKaymiyor = !kaydirici;
  const alanSinirda = kaydirici && (
    (asagi && kaydirici.scrollTop + kaydirici.clientHeight >= kaydirici.scrollHeight - 1) ||
    (!asagi && kaydirici.scrollTop <= 1)
  );

  if (sayiInput && document.activeElement === sayiInput) {
    sayiInput.blur();
  }

  if (sayiInput || tabloIci || alanDikeyKaymiyor || alanSinirda) {
    e.preventDefault();
    window.scrollBy({
      top: e.deltaY,
      left: 0,

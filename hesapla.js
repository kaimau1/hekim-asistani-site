...
const HYP2_TURKIYE_ORT_NUFUS_PUANI_ORANI = 0.9104;
const HYP2_MAAS_FARKI_CARPANI = 25.5;
      ...
      tr.innerHTML =
        ...
        `<td data-kolon="hedef" class="hedef-cell">` +
          `<div class="bar-tum" id="hypV2_${k.prefix}_BT">` +
            `<div class="bar-ust">` +
              `<span class="bar-sayilan" id="hypV2_${k.prefix}_S" title="Sayılan">—</span>` +
...
function _localStorageYukle(opts = {}) {
  let d;
  try {
    let raw = opts.forceLegacy ? localStorage.getItem(_LS_HESAP_KEY) : localStorage.getItem(_localStorageAktifKey());
    if (!raw && opts.temizleYoksa) {
  ...
  if (!d) return;
  if (_aktifHesapUid && opts.forceLegacy) {
...
window.hesapAktifUidAyarla = function(uid) {
  _aktifHesapUid = uid || null;
  _localStorageYukle({ temizleYoksa: true });
  if (typeof hypV2Hesapla === 'function') hypV2Hesapla();
  if (typeof ascHesapla === 'function') ascHesapla();
  _localStorageYaz();
};
...
function _v2NufusPuaniHesapla(nufus) {
  const el = document.getElementById('hypV2NufusPuani');
  const manuel = parseFloat(String(el?.value || '').replace(',', '.')) || 0;
  if (manuel > 0) return { puan: manuel, kaynak: 'Elle girildi' };
  if (nufus <= 0) return { puan: 0, kaynak: 'Türkiye ortalaması' };
  return { puan: nufus * HYP2_TURKIYE_ORT_NUFUS_PUANI_ORANI, kaynak: 'Türkiye ortalaması' };
}

function _v2MaasFarkiGuncelle({ nufus, tavanKS, maasKS }) {
  const farkEl = document.getElementById('hypV2MaasFarki');
  const kaynakEl = document.getElementById('hypV2MaasFarkiKaynak');
  if (!farkEl) return;

  const { puan, kaynak } = _v2NufusPuaniHesapla(nufus);
  const katsayiFarki = Math.max(0, tavanKS - maasKS);
  const maasFarki = puan * katsayiFarki * HYP2_MAAS_FARKI_CARPANI;
  ...
  farkEl.textContent = nufus > 0 ? paraFmt.format(maasFarki) : '—';
  farkEl.classList.toggle('tavanda', nufus > 0 && katsayiFarki <= 0.0001);
  if (kaynakEl) {
    kaynakEl.textContent = nufus > 0
      ? `${kaynak} · ${puan.toLocaleString('tr-TR', { maximumFractionDigits: 2 })} puan`
      : kaynak;
  }
}
...
function hypV2Hesapla() {
  ...
  HYP2_KRITERLER.forEach(k => {
    ...
    if (sayilanEl) {
      ...
      sayilanEl.className = 'bar-sayilan' + (G > 0 && oran >= tablo.azami ? ' iyi' : G > 0 && oran < tablo.asgari ? ' kotu' : '');
    }
    if (pctEl) {
  ...
  _v2MaasFarkiGuncelle({ nufus, tavanKS, maasKS });
  if (durumEl) {
  ...
  ASC_KRITERLER.forEach(k => {
    ...
    tr.innerHTML =
      ...
      `<td data-kolon="hedef" class="hedef-cell">` +
          ...
          `<div class="bar-ust">` +
            `<span class="bar-sayilan" id="asc_${k.prefix}_S" title="Sayılan">—</span>` +
...
function ascHesapla() {
  ...
  ASC_KRITERLER.forEach(k => {
    ...
    if (sayilanEl) {
      sayilanEl.textContent = G > 0 ? String(Y + D) : '—';
      sayilanEl.className = 'bar-sayilan' + (G > 0 && oran >= t.azami ? ' iyi' : G > 0 && oran < t.asgari ? ' kotu' : '');
    }
    if (pctEl) {
      pctEl.textContent = G > 0 ? Math.round(oran) + '%' : '—';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  hypV2ProfilKur();
  ascProfilKur();
  _localStorageYukle({ temizleYoksa: true });       // v: aktif yerel çalışma alanından devam et
  hypV2Hesapla();
  ascHesapla();
  ...
  window.addEventListener('hesap-eklenti-veri-geldi', () => {
    ...
    _localStorageYukle({ forceLegacy: true });

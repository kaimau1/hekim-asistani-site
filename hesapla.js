...
function hypV2TabloKur() {
  ...
  HYP2_GRUPLAR.forEach(grup => {
    ...
    grupKriterler.forEach((k) => {
      ...
      tr.innerHTML =
        `<td data-kolon="kriter"><span class="drag-handle" title="Sürükle-bırak ile sırala">⋮⋮</span><span class="kriter-ad"><span class="hedef-gerekmez-uyari" hidden>Gerek yok</span>${k.ad}</span></td>` +
...
function _barCiz(container, { G, Y, D, hedef, asgari, azami }) {
  ...
  const hedefTavanAyni = hedefPct > 0 && Math.abs(hedefPct - azami) < 0.1;
  const hedefEtiketi = hedefPct > 0
    ? `<span class="bar-et et-hedef${hedefUlasildi ? ' ulasildi' : ''}" style="left:${hedefPct}%"><span class="et-deger">${hedef}${hedefUlasildi ? ' ✓' : ''}</span></span>`
    : '';
  const tavanEtiketi = hedefTavanAyni
    ? ''
...
function hypV2Hesapla() {
  ...
  const khtBilgiEl = document.getElementById('hypV2KhtBilgiNotu');
  if (ger40El && ger70El) {
    if (gT > 0) {
      ...
      if (khtBilgiEl) {
        const metin40 = ger40 > 0 ? `Yüzde 40 için ${ger40} tane lazım` : 'Yüzde 40 tamam';
        const metin70 = ger70 > 0 ? `yüzde 70 için ${ger70} tane lazım` : 'yüzde 70 tamam';
        khtBilgiEl.textContent = `${metin40} · ${metin70}`;
        khtBilgiEl.hidden = false;
      }
    } else {
      ger40El.textContent = '';
      ger70El.textContent = '';
      if (khtBilgiEl) {
        khtBilgiEl.textContent = '';
        khtBilgiEl.hidden = true;
      }
    }
  }
}
...
function ascTabloKur() {
  ...
  ASC_KRITERLER.forEach(k => {
    ...
    tr.dataset.prefix = k.prefix;
    tr.innerHTML =
      `<td data-kolon="kriter"><span class="drag-handle" title="Sürükle-bırak ile sırala">⋮⋮</span><span class="kriter-ad"><span class="hedef-gerekmez-uyari" hidden>Gerek yok</span>${k.ad}</span><div class="ad-kisa">${k.alt}</div></td>` +
...
function _hedefGerekmezUyariGuncelle(prefix, goster) {
  document.querySelectorAll(`tr[data-prefix="${prefix}"]`).forEach(tr => {
    const uyari = tr.querySelector('.hedef-gerekmez-uyari');
    if (!uyari) return;
    uyari.hidden = !goster;

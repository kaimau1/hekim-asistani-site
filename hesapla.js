...
function _otomatikHedefSinirlariniHesapla({ G, Y, maxEtkin, asgari, isMecbur }) {
  const asgariMinY = G > 0 ? Math.ceil(G * asgari / 100) : 0;
  const minY = asgariMinY;
  const maxEtkinY = G > 0 ? Math.ceil(G * maxEtkin / 100) : 0;

  let hedefY;
  if (isMecbur) {
    if (maxEtkinY < asgariMinY) {
      hedefY = 0;
    } else if (Y > maxEtkinY) {
      hedefY = maxEtkinY;
    } else {

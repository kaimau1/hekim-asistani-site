...
function _barCiz(container, { G, Y, D, hedef, asgari, azami }) {
  ...
  container.style.setProperty('--hedef-sol', hedefPct > 0 ? hedefPct.toFixed(1) + '%' : '100%');
  if (inp) {
  ...
  const barBilgi = G > 0
    ? `Asgari değer: ${asgari}% (${asgariY} kişi) · Hedef değer: ${hedefPct > 0 ? `${hedefPct.toFixed(1)}% (${hedef} kişi)` : 'belirlenmedi'} · Tavan değer: ${azami}% (${azamiY} kişi)`
  ...
  const hedefAsgariyeYakin = hedefPct > 0 && Math.abs(hedefPct - asgari) < hedefCakismaMesafesi;
  const hedefTavanaYakin = hedefPct > 0 && Math.abs(hedefPct - azami) < hedefCakismaMesafesi;
  const hedefAltEtiketiGoster = hedefPct > 0 && !hedefAsgariyeYakin && !hedefTavanaYakin;
  const hedefEtiketi = hedefAltEtiketiGoster
    ...
    : '';
  const tavanEtiketi = `<span class="bar-et et-tavan${tavanUlasildi ? ' ulasildi' : ''}" style="left:${azami}%"><span class="et-deger">${azamiY}${tavanUlasildi ? ' ✓' : ''}</span></span>`;
  alt.innerHTML =

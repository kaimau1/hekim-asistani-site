...
function _yeniAyUyarisiGuncelle() {
  ...
  const oncekiGerekenBtn = document.getElementById('hypV2OncekiAyGereken');
  const metin = uyari.querySelector('[data-ay-uyari-metin]');
  const yeniAyUyarisiGoster = _aktifHesapAy === oncekiAy && buAy !== oncekiAy;
  uyari.hidden = !yeniAyUyarisiGoster;
  if (metin) metin.textContent = `${_ayEtiketi(oncekiAy)} verileri acik. ${_ayEtiketi(buAy)} ayina gecmek ister misiniz?`;
  if (yeniAyBtn) yeniAyBtn.dataset.ay = buAy;
  if (oncekiGerekenBtn) {
    oncekiGerekenBtn.hidden = _aktifHesapAy !== buAy || !_aylikKaydiOku(_oncekiAyAnahtari(_aktifHesapAy));
  }
}

function _oncekiAyGerekenleriniKullan() {
  const onceki = _aylikKaydiOku(_oncekiAyAnahtari(_aktifHesapAy))?.veri;
  if (!onceki?.hypV2) return;
  ...
  try {
    ...
    if (nufusEl && Object.prototype.hasOwnProperty.call(onceki.hypV2, 'nufus')) nufusEl.value = onceki.hypV2.nufus || '';
    const nufusTipEl = document.getElementById('hypV2NufusTip');
    if (nufusTipEl && onceki.hypV2.nufusTip) nufusTipEl.value = onceki.hypV2.nufusTip;
    const nufusPuaniEl = document.getElementById('hypV2NufusPuani');
    if (nufusPuaniEl && Object.prototype.hasOwnProperty.call(onceki.hypV2, 'nufusPuani')) nufusPuaniEl.value = onceki.hypV2.nufusPuani || '';
    HYP2_KRITERLER.forEach(k => {
      const kaynak = onceki.hypV2.kriterler?.[k.prefix] || {};
      const gEl = document.querySelector(`input[data-hypv2="${k.prefix}-G"]`);
      const yEl = document.querySelector(`input[data-hypv2="${k.prefix}-Y"]`);
      const dEl = document.querySelector(`input[data-hypv2="${k.prefix}-D"]`);
      if (gEl && Object.prototype.hasOwnProperty.call(kaynak, 'g')) gEl.value = kaynak.g || '';
      if (yEl && !yEl.value) yEl.value = '';
      if (dEl && !dEl.value) dEl.value = '';
    });
  } finally {
  ...
  try {
    window.dispatchEvent(new CustomEvent('hesap-verisi-degisti', {
      detail: { kaynak: 'yerel', ayAnahtari: _aktifHesapAy }
    }));
  } catch (_) {}
  if (typeof window.siteToast === 'function') window.siteToast('Onceki ay gerekenleri bu aya alindi', 'basari');
  _aySeciciGuncelle();
}
...
const _HESAP_ALAN_SECICI = '#hypV2Nufus, #hypV2NufusTip, #hypV2NufusPuani, ' +
  '#ascHekimKS, #ascNufus, #ascNufusTip, ' +
...
window.hesapFormDoluMu = function() {
  ...
  if ((parseInt(document.getElementById('ascNufus')?.value) || 0) > 0) return true;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('hypV2OncekiAyGereken')?.addEventListener('click', _oncekiAyGerekenleriniKullan);

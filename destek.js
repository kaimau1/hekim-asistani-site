...
const DOM = {
  ...
  destekEmailKopyala: document.getElementById('destekEmailKopyala'),
  destekEmailKopyaDurum: document.getElementById('destekEmailKopyaDurum'),
};
...
async function destekEmailKopyala() {
  const email = DOM.destekEmailKopyala?.dataset?.destekEmail || 'destek@ahekplus.com';
  try {
    await navigator.clipboard.writeText(email);
    if (DOM.destekEmailKopyaDurum) DOM.destekEmailKopyaDurum.textContent = 'Kopyalandı';
    clearTimeout(DOM.destekEmailKopyaDurum?._t);
    if (DOM.destekEmailKopyaDurum) {
      DOM.destekEmailKopyaDurum._t = setTimeout(() => {
        DOM.destekEmailKopyaDurum.textContent = '';
      }, 1800);
    }
  } catch (_) {
    if (DOM.destekEmailKopyaDurum) DOM.destekEmailKopyaDurum.textContent = email;
  }
}
...
function olaylariBagla() {
  DOM.destekEmailKopyala?.addEventListener('click', destekEmailKopyala);

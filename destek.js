...
const DOM = {
  ...
  destekGirisRozet: document.getElementById('destekGirisRozet'),
...
function anaGorunumuAyarla(girisVarMi) {
  ...
  if (DOM.destekGirisRozet) DOM.destekGirisRozet.hidden = girisVarMi;
}
...
function ticketListesiniCiz() {
  ...
  DOM.destekTalepListesi.innerHTML = sirali.map((talep) => {
    ...
    const kullaniciBilgi = isAdmin
      ...
      : '';
    return `
      <button type="button" class="destek-talep-item${aktif}" data-talep-id="${talep.id}" data-talep-yol="${talep.kaynakYol || ''}">

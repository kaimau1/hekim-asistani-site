...
const DOM = {
  destekGirisKarti: document.getElementById('destekGirisKarti'),
  destekGirisBaglanti: document.getElementById('destekGirisBaglanti'),
...
function anaGorunumuAyarla(girisVarMi) {
  if (DOM.destekGirisKarti) DOM.destekGirisKarti.hidden = girisVarMi;
...
function mesajlariCiz() {
  ...
  DOM.destekMesajListesi.innerHTML = aktifMesajlar.map((mesaj) => {
...
async function yeniTalepOlustur(e) {
  e.preventDefault();
  if (!aktifUid || !aktifKullanici) return;
...
async function yanitGonder(e) {
  e.preventDefault();
  if (!aktifUid || !aktifKullanici || !aktifTalepId) return;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  onAuthStateChanged(auth, (user) => {
    if (!user) {
      ...
      anaGorunumuAyarla(false);
      if (DOM.destekGirisKarti) DOM.destekGirisKarti.hidden = false;
      if (DOM.destekGirisBaglanti) DOM.destekGirisBaglanti.href = 'giris.html?sonra=destek.html';
      return;
    }
    ...
    if (DOM.destekGirisKarti) DOM.destekGirisKarti.hidden = true;
